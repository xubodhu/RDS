...
class BertCRF(BertPreTrainedModel):
    ...
    def forward(self, input_ids, segment_ids, input_mask, labels=None):
        ...
        if labels is not None:
            loss = - self.crf(bert_feats, labels, mask=input_mask.byte(), reduction='mean')
            return loss
        else:
...
class MTCCMBertForMMTokenClassificationCRF(BertPreTrainedModel):
    ...
    def forward(self, input_ids, segment_ids, input_mask, added_attention_mask, visual_embeds_att, trans_matrix,
        ...
        sequence_output = self.dropout(sequence_output)
        extended_txt_mask = input_mask.unsqueeze(1).unsqueeze(2) # input_mask: batch_size * seq_len
        extended_txt_mask = extended_txt_mask.to(dtype=next(self.parameters()).dtype) # fp16 compatibility
        extended_txt_mask = (1.0 - extended_txt_mask) * -10000.0    # extended_txt_mask: batch_size * 1 * 1 * seq_len
        ...
        aux_bert_feats = self.aux_classifier(aux_addon_sequence_output) # batch_size * seq_len * aux_label_nums（7）


        trans_bert_feats = torch.matmul(aux_bert_feats, trans_matrix.float())   # batch_size * seq_len * label_nums(13)

        main_addon_sequence_encoder = self.self_attention_v2(sequence_output, extended_txt_mask)
        main_addon_sequence_output = main_addon_sequence_encoder[-1]    # batch_size * seq_len * hidden_size
        ...
        # visual gate
        merge_representation = torch.cat((cross_final_txt_layer, cross_output_layer), dim=-1)   # batch_size * seq_len * 2(hidden_size)
        gate_value = torch.sigmoid(self.gate(merge_representation))  # batch_size, text_len, hidden_dim
        gated_converted_att_vis_embed = torch.mul(gate_value, cross_output_layer)   # batch_size, text_len, hidden_dim

        final_output = torch.cat((cross_final_txt_layer, gated_converted_att_vis_embed), dim=-1) # batch_size * seq_len * 2(hidden_size)
        bert_feats = self.classifier(final_output)  # batch_size * seq_len * 13

        alpha = 0.5
        final_bert_feats = torch.add(torch.mul(bert_feats, alpha),torch.mul(trans_bert_feats, 1-alpha)) # batch_size * seq_len * 13
