...
class SentenceREDataset(data.Dataset):
    ...
    def __init__(self, text_path, rel_path, pic_path, rel2id, tokenizer, kwargs):
        ...
        # 使用CLIP处理器来处理数据
        self.clip_preprocessor = clip_preprocessor = CLIPProcessor.from_pretrained("../pretrained_models/CLIP_model")
    ...
    def __getitem__(self, index):
        ...
        pic = self.img_dict[item['img_id']]  # 10 * 4096
        rel = self.rel_dict[item['img_id']]  # 128 * 10
        rel = self.padding(item, rel, pos1, pos2)  # 128 * 10
        ...
        list_p = list(pic1)  # 1 * 10 * 4096
        list_r = list(rel1)  # 1 * 128 * 10

        # # clip处理器处理数据
        text = ' '.join(item['token'])
        img_path = ''
        if 'train' in self.text_path:
            img_path = './mnre_img/img_org/train/' + item['img_id']
        elif 'val' in self.text_path:
            img_path = './mnre_img/img_org/val/' + item['img_id']
        elif 'test' in self.text_path:
            img_path = './mnre_img/img_org/test/' + item['img_id']
        if not os.path.exists(img_path):
            print(img_path + "not exits")

        try:
            image = Image.open(img_path).convert("RGB")
        except:
            img_path_fail = './mnre_img/img_org/17_06_4705.jpg'
            image = Image.open(img_path_fail).convert("RGB")


        clip_result = self.clip_preprocessor(text=text, images=image, padding='max_length',
                                     max_length=64, truncation=True)
        clip_input_ids = clip_result['input_ids']   # 64
        clip_attention_mask = clip_result['attention_mask'] # 64
        clip_pixel_values = clip_result['pixel_values'][0]  # 3 * 224 * 224
        clip_input_ids = torch.tensor(clip_input_ids).unsqueeze(0).unsqueeze(0)
        clip_attention_mask = torch.tensor(clip_attention_mask).unsqueeze(0).unsqueeze(0)
        clip_pixel_values = torch.tensor(clip_pixel_values).unsqueeze(0).unsqueeze(0)



        clip_input_ids = list(clip_input_ids)
        clip_attention_mask = list(clip_attention_mask)
        clip_pixel_values = list(clip_pixel_values)

        res = [self.rel2id[item['relation']]] + seq + list_p + list_r \
              + clip_input_ids + clip_attention_mask + clip_pixel_values
    ...
    def collate_fn(data):
        # data : [label, seq1, seq2, ..., pic] [label, seq1, seq2, ..., pic]
        # -> [label, label, label], [seq1, seq1, seq1], ...
        data = list(zip(*data))
        labels = data[0]
        seqs = data[1:]
